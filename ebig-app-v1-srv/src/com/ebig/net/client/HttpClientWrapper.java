...
public class HttpClientWrapper {
    ...
    private List<BasicHeader> headerBodies; 
    ...
    public HttpClientWrapper() {
        ...
        this.headerBodies 	     = new ArrayList<BasicHeader>();
    ...
    public ResponseContent getResponse(String urlstr, String urlEncoding, VERBTYPE bodyType, String contentType)
        ...
        try {
            if (VERBTYPE.GET == bodyType) {
                ...
                this.parseUrl(url);
                HttpPostHC4 httpPost = new HttpPostHC4(toUrl());
            ...
            Header headerss[] = this.getNVHeaderss();  
            if(headerss != null && headerss.length > 0){  
            	request.setHeaders(headerss);  
            }  
       
            response = client.execute(request);                       
            response.addHeader(HttpHeaders.ACCEPT_CHARSET,"GBK");
    ...
    private Header[] getNVHeaderss() {
    	if(this.headerBodies != null && this.headerBodies.size() > 0 ){
    		return this.headerBodies.toArray(new Header[this.headerBodies.size()]);
    	}    		
		return null;
	}
    ...
    private void close(HttpEntity entity, HttpRequestBaseHC4 request, CloseableHttpResponse response) throws IOException {
        if (request != null)
            request.abort();
        if (entity != null)
	...
	public void buildGetUrl( Map<String, String> params) {		
		if (params != null) {
			List<NameValuePair> ps = new ArrayList<NameValuePair>();
			for (String key : params.keySet()) {				
				addNV(key, params.get(key));
			}			
		}		
	}
	
    public void addNVHeader(String name, String value) {
    	BasicHeader nvh = new BasicHeader(name,value);  
        this.headerBodies.add(nvh);
    }
 
    public void clearNVHeaders() {
        this.headerBodies.clear();
    }
    

   public void buildHeader(Map<String,String> params){  	   
		if (params != null) {
			List<BasicHeader> ps = new ArrayList<BasicHeader>();
			for (String key : params.keySet()) {				
				addNVHeader(key, params.get(key));
			}			
		}
	}  
