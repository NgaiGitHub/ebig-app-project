...
package com.tpos.widget.pagertab;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int tabTextSize = 11;// steven update  原来是16
	...
	public void notifyDataSetChanged() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (pager.getAdapter() instanceof IconTabProvider) {
				...
				// steven add 
				addIconTextTab(i, ((IconTextTabProvider) pager.getAdapter()).getPageIconResId(i), pager.getAdapter().getPageTitle(i).toString());
			} else {
	...
	private void addIconTextTab(final int position, int resId,String title) {
		...
		if(tDrawable != null){ 
			tDrawable.setBounds(0, 0, (int) (tDrawable.getIntrinsicWidth() * 0.5), (int) (tDrawable.getIntrinsicHeight() * 0.5));			
...
//				LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT, 1.0f));
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if(pager.getAdapter() instanceof IconTextTabProvider){
				if(v instanceof TextView){
					Drawable tDrawable = null;
					try {
						tDrawable = getResources().getDrawable(((IconTextTabProvider) pager.getAdapter()).getPageIconResId(i));
					} catch (NotFoundException e) {
						e.printStackTrace();
						tDrawable = null;
					}
	...
	// steven add
	public void setDefaultPage(int position){
		pager.setCurrentItem(position);
		if(position == 0)
			setSelectedTabIcon(position, ((IconTextTabProvider) pager.getAdapter()).getPageIconSelResId(position));
			//setSelectedTextColor(position, underlineColor);
	}
...
//				setSelectedTabIcon(position, ((IconTextTabProvider) pager.getAdapter()).getPageIconSelResId(position));
	 ...
	 * 设置选择字体颜色
	 */
	private void setSelectedTextColor(int position,int color){
		// update steven
		updateTabStyles();
		View v = tabsContainer.getChildAt(position);
		if(v instanceof TextView){
